...
up_lim = double(S);
csv_filename = ['data_for', num2str(y_flag), 'cm_calculation.csv'];
clear q_ar x_ar
x_ar(1)=1;
q_ar(1)=1;
...
data = [x_ar', q_ar'];
csv_filename = ['data_for', num2str(y_flag), 'cm_calculation_1_2_eq.csv'];
...
q_ar1(1)=[]; %clean q
x_ar1(1)=[];
q_ar1 = q_ar1/q_ar1(end);
plot(x_ar1, q_ar1)
legend('Eq 1 + 2','Eq 3','Location','southeast')
text_desc = ['Values of Cumulative Volume in function of distance for Y value of:', num2str(y_flag), ' cm']
title(text_desc)
csv_filename = ['data_for', num2str(y_flag), 'cm_calculation_3_eq.csv'];
data = [x_ar1', q_ar1'];
csvwrite(csv_filename, data)